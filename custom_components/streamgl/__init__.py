...
from homeassistant.components.stream.const import RECORDER_PROVIDER
...
def async_register_camera_entity_service(
    ...
    def get_entities() -> dict[str, Entity]:
        cam_entities = {}
        for (et, _), et_dict in hass.data.get(DATA_DOMAIN_PLATFORM_ENTITIES, {}).items():
            if et == CAMERA_DOMAIN:
...
async def async_setup(hass: HomeAssistant, _config: ConfigType) -> bool:
    ...
    # register camera start / stop record services
...
def _set_camera_recording(camera: Camera, recording: bool) -> None:
    camera._attr_is_recording = recording  # noqa: SLF001
...
async def _snapshot(camera: Camera, snap_file: str, tnb_file: str) -> None:
    ...
    if snap_image is None:
        _LOGGER.warning("Cannot generate snap image")
        return

    def _write_image(to_file: str, image_data: bytes) -> None:
        """Executor helper to write image."""
        Path.mkdir(Path(to_file).parent, parents=True, exist_ok=True)
        with Path(to_file).open("wb") as img_file:
    ...
    try:
        ...
        reason = f"Can't write image to file: {err}"
        raise HomeAssistantError(reason) from err


async def async_handle_start_recording_service(camera: Camera, service_call: ServiceCall) -> None:
    """Handle stream recording service calls. adapted from async_handle_record_service."""
    if camera.is_recording:
        reason = f"{camera.entity_id} is already recording"
    ...
    try:
        if CONF_GALLERY in service_call.data:
            ...
            service_call.data = new_data  # type: ignore  # noqa: PGH003
...
async def async_handle_stop_recording_service(camera: Camera, _service_call: ServiceCall) -> None:
